...
--[[
    Draggable Toggle Icon – always on top, square with border, and background blur.
    Clique = toggle UI, arraste = mover o ícone.
]]
function NeverLose:CreateToggleIcon(Window, Config)
    Config = NeverLose:ProcessParams(Config, {
        ...
        Blur   = true,   -- blur ativo por padrão
    })
    ...
    local UIStroke = Instance.new("UIStroke")
    local ImageLabel = Instance.new("ImageLabel")
    local Shadow = NeverLose:CreateShadow(IconFrame, true)  -- glow
    ...
    IconFrame.Active = true   -- necessário para receber eventos de entrada
    ...
    -- ===== LÓGICA DE ARRASTE E CLIQUE =====
    local startMousePos = nil   -- posição onde o clique começou
    local startFramePos = nil   -- posição do frame quando o clique começou
    ...
    IconFrame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            startFramePos = IconFrame.Position
            isDragging = false
        end
    end)

    -- Movimento do mouse (enquanto segura o botão)
    UserInputService.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement and startMousePos then
            local delta = input.Position - startMousePos
            if delta.Magnitude > 2 then
                isDragging = true
            end
            if isDragging then
                -- Atualiza a posição do frame com o delta
    ...
    IconFrame.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            -- Se não houve arraste significativo, dispara o toggle
    ...
    IconFrame.MouseEnter:Connect(hoverIn)
    IconFrame.MouseLeave:Connect(hoverOut)

    -- ===== BLUR ATRÁS DO ÍCONE (sempre ativo) =====
    if Config.Blur then
