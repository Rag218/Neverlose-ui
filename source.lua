...
-- Cria o ícone arrastável
local function CriarIconeArrastavel()
    local Icone = Instance.new("Frame")
    Icone.Size = UDim2.new(0, 60, 0, 60)
    Icone.Position = UDim2.new(0, 100, 0, 100)
    Icone.BackgroundColor3 = Color3.fromRGB(20, 22, 27)
    Icone.BackgroundTransparency = 0.1
    Icone.BorderSizePixel = 0
    Icone.ClipsDescendants = true
    Icone.ZIndex = 20
    Icone.Parent = NeverLose.ScreenGui

    -- Bordas (quadrado com cantos arredondados)
    local Borda = Instance.new("UIStroke", Icone)
    Borda.Thickness = 2
    Borda.Color = Color3.fromRGB(255, 255, 255)
    ...
    -- O ícone propriamente dito (pode ser uma imagem ou texto)
    local Imagem = Instance.new("ImageLabel", Icone)
    Imagem.Size = UDim2.new(0.8, 0, 0.8, 0)
    Imagem.Position = UDim2.new(0.1, 0, 0.1, 0)
    Imagem.BackgroundTransparency = 1
    Imagem.Image = "rbxassetid://120358385035996" -- teu logo
    Imagem.ScaleType = Enum.ScaleType.Fit

    -- Ativa o arraste (usa a função nativa da NeverLose)
    NeverLose.Drag(Icone, Icone, 0.15)

    -- Aplica o blur atrás do ícone (exatamente como na janela principal)
    local BlurSignal = NeverLose:CreateSignal(true) -- sempre ativo
    NeverLose:CreateBlurModule(Icone, BlurSignal)

    -- Para desativar o blur quando quiser: BlurSignal:SetValue(false)

    return Icone
end

-- Chama a função para criar
local MeuIcone = CriarIconeArrastavel()
