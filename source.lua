...
-- ========== DYNAMIC THEMING REGISTRY ==========
NeverLose._uiElements = {
    toggles = {},
    sliders = {},
    colorPickers = {},
    toggleIcons = {},
    misc = {}  -- for future extensions
}

-- ========== THEME REFRESH FUNCTION ==========
function NeverLose.RefreshTheme(newColor)
    local accent = newColor or NeverLose.AccentColor
    NeverLose.AccentColor = accent

    -- Update toggles (active background)
    for _, toggle in ipairs(NeverLose._uiElements.toggles) do
        if toggle.frame then
            if toggle.active then
                toggle.frame.BackgroundColor3 = accent
            else
                toggle.frame.BackgroundColor3 = Color3.fromRGB(10, 13, 21) -- default off
            end
        end
    end

    -- Update sliders (fill & knob)
    for _, slider in ipairs(NeverLose._uiElements.sliders) do
        if slider.fill then
            slider.fill.BackgroundColor3 = accent
        end
        if slider.knob then
            slider.knob.BackgroundColor3 = accent
        end
    end

    -- Update color picker preview buttons (the small swatch)
    for _, cp in ipairs(NeverLose._uiElements.colorPickers) do
        if cp.button then
            cp.button.BackgroundColor3 = accent
        end
    end

    -- Update toggle icons (stroke & image color)
    for _, icon in ipairs(NeverLose._uiElements.toggleIcons) do
        if icon.stroke then
            icon.stroke.Color = accent
        end
        if icon.image then
            icon.image.ImageColor3 = accent
        end
    end
end
...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddToggle(Config)
		...
		-- ========== REGISTER FOR DYNAMIC THEMING ==========
		local toggleObj = {
			frame = Toggle,
			active = Config.Default
		}
		table.insert(NeverLose._uiElements.toggles, toggleObj)

		NeverLose:CreateInput(Toggle , LPH_NO_VIRTUALIZE(function()
			Config.Default = not Config.Default;
			toggleObj.active = Config.Default   -- update registry
			ToggleLib.SetUI(Config.Default);
		...
		function ToggleLib:SetValue(v)
			Config.Default = v;
			toggleObj.active = v   -- keep registry in sync
	...
	function handle:AddSlider(Config)
		...
		local UICorner_4 = Instance.new("UICorner")
		local Frame = Instance.new("Frame")   -- this is the knob
		local UICorner_5 = Instance.new("UICorner")
		local boxSize = 2;
		...
		table.insert(NeverLose._uiElements.sliders, sliderObj)
	...
	function handle:AddColorPicker(Config)
		...
		table.insert(NeverLose._uiElements.colorPickers, cpObj)
...
--[[
    Draggable Toggle Icon – funciona em mobile (touch) e PC.
    Clique curto → toggle da janela.
    Arraste → move o ícone.
]]
function NeverLose:CreateToggleIcon(Window, Config)
    Config = NeverLose:ProcessParams(Config, {
        Icon   = NeverLose.GlobalLogo,
        Size   = 50,
        Border = 2,
    ...
    local UIStroke = Instance.new("UIStroke")
    local ImageLabel = Instance.new("ImageLabel")
    local Shadow = NeverLose:CreateShadow(IconFrame, true)

    IconFrame.Name = NeverLose.RandomString()
    IconFrame.Parent = NeverLose.ScreenGui
    ...
    IconFrame.Size = UDim2.fromOffset(Config.Size, Config.Size)
    IconFrame.Position = UDim2.fromOffset(20, 20)
    IconFrame.ZIndex = 999   -- sempre no topo

    UICorner.CornerRadius = UDim.new(0, 8)
    UICorner.Parent = IconFrame

    UIStroke.Thickness = Config.Border
    ...
    table.insert(NeverLose._uiElements.toggleIcons, iconObj)

    -- Frame para arraste e clique (substitui o ImageButton)
    local DragFrame = Instance.new("Frame")
    DragFrame.Parent = IconFrame
    ...
    -- Inicia o arraste ou registra o clique
    ...
    -- Ao soltar, decide se foi clique ou arraste
    DragFrame.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if startPos and not isDragging then
                local delta = (input.Position - startPos).Magnitude
                if delta < 10 then  -- clique curto (sem movimento significativo)
                    Window:ToggleInterface()
                end
            end
            startPos = nil
    ...
    -- Detecta movimento para marcar como arraste
    ...
    -- Aplica o arraste (reutiliza a função Drag do NeverLose)
    -- Passamos o DragFrame como InputFrame e o IconFrame como MoveFrame
    NeverLose.Drag(DragFrame, IconFrame, 0.15)

    -- Efeitos hover (mouse)
    local function hoverIn()
        NeverLose.PlayAnimate(IconFrame, SlowyTween, {
            BackgroundTransparency = 0.05,
            Size = UDim2.fromOffset(Config.Size + 4, Config.Size + 4)
        })
        NeverLose.PlayAnimate(UIStroke, SlowyTween, { Transparency = 0.1 })
    end
    local function hoverOut()
        ...
        NeverLose.PlayAnimate(UIStroke, SlowyTween, { Transparency = 0.3 })
    end
    DragFrame.MouseEnter:Connect(hoverIn)
    DragFrame.MouseLeave:Connect(hoverOut)

    -- Blur atrás do ícone (opcional)
    local blurPart = Instance.new("Part")
    local depth = Instance.new("DepthOfFieldEffect", game:GetService("Lighting"))
    blurPart.Anchored = true
    blurPart.Transparency = 1
    blurPart.CanCollide = false
    blurPart.Size = Vector3.new(1, 1, 1) * 0.1
    blurPart.Parent = workspace.CurrentCamera
    depth.Enabled = true
    depth.FarIntensity = 0
    depth.InFocusRadius = 1000
    depth.NearIntensity = 0.85

    local function updateBlur()
        local pos = IconFrame.AbsolutePosition
        local size = IconFrame.AbsoluteSize
        local center = pos + size / 2
        local ray = workspace.CurrentCamera:ViewportPointToRay(center.X, center.Y, 1)
        local worldPos = ray.Origin + ray.Direction * 5
        blurPart.CFrame = CFrame.new(worldPos)
        blurPart.Size = Vector3.new(size.X / 100, size.Y / 100, 0.1)
    end
    updateBlur()
    game:GetService("RunService").RenderStepped:Connect(updateBlur)

    IconFrame.AncestryChanged:Connect(function()
        if not IconFrame.Parent then
            blurPart:Destroy()
            depth:Destroy()
        end
    end)
    ...
    return {
        Frame = IconFrame,
        Image = ImageLabel,
        SetPosition = function(x, y) IconFrame.Position = UDim2.fromOffset(x, y) end,
        SetIcon = function(asset) ImageLabel.Image = asset end,
        SetColor = function(color) UIStroke.Color = color; ImageLabel.ImageColor3 = color end,
        Destroy = function() IconFrame:Destroy() end,
    }
end
