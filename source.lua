...
-- ====================================================
-- Ícone Flutuante com Blur e Drag (Toggle UI)
-- ====================================================
local function CriarIconeFlutuante(window)
    -- Cria o frame do ícone
    local icone = Instance.new("Frame")
    icone.Name = "IconeFlutuante"
    icone.Parent = NeverLose.ScreenGui  -- sempre no topo
    icone.AnchorPoint = Vector2.new(0, 0)
    icone.Position = UDim2.new(0, 20, 0, 20)
    icone.Size = UDim2.new(0, 50, 0, 50)
    icone.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
    icone.BackgroundTransparency = 0.15
    icone.BorderSizePixel = 2
    icone.BorderColor3 = Color3.fromRGB(78, 127, 252)
    icone.ClipsDescendants = true
    icone.ZIndex = 999  -- sempre acima de tudo

    -- Cantos arredondados
    local corner = Instance.new("UICorner", icone)
    corner.CornerRadius = UDim.new(0, 8)

    -- Imagem (pode ser qualquer asset)
    local imagem = Instance.new("ImageLabel", icone)
    imagem.AnchorPoint = Vector2.new(0.5, 0.5)
    imagem.Position = UDim2.new(0.5, 0, 0.5, 0)
    imagem.Size = UDim2.new(0.8, 0, 0.8, 0)
    imagem.BackgroundTransparency = 1
    imagem.Image = "rbxassetid://120358385035996"  -- teu logo
    imagem.ImageColor3 = Color3.fromRGB(255,255,255)

    -- Sombra / Glow (UIStroke com transparência)
    local stroke = Instance.new("UIStroke", icone)
    stroke.Thickness = 2
    stroke.Color = Color3.fromRGB(78, 127, 252)
    stroke.Transparency = 0.3

    -- ======== BLUR EXTERNO ========
    -- Cria um blur via DepthOfField focado apenas no ícone
    local blurPart = Instance.new("Part")
    blurPart.Name = "BlurIcone"
    blurPart.Anchored = true
    blurPart.CanCollide = false
    blurPart.Transparency = 1
    blurPart.Size = Vector3.new(2, 2, 0.5)
    blurPart.CFrame = CFrame.new(0, 0, -10)
    blurPart.Parent = workspace.CurrentCamera

    local blurEffect = Instance.new("DepthOfFieldEffect", game:GetService("Lighting"))
    blurEffect.Name = "BlurIconeEffect"
    blurEffect.Enabled = true
    blurEffect.FarIntensity = 0
    blurEffect.NearIntensity = 0.8
    blurEffect.FocusDistance = 0
    blurEffect.InFocusRadius = 1000

    -- Atualiza posição do blur acompanhando o ícone
    local function updateBlur()
        local pos = icone.AbsolutePosition
        local size = icone.AbsoluteSize
        local center = pos + size/2
        -- Converte para coordenadas 3D (simplificado)
        local ray = CurrentCamera:ScreenPointToRay(center.X, center.Y, 0)
        local planePos = CurrentCamera.CFrame.Position + CurrentCamera.CFrame.LookVector * 3
        local hit = NeverLose.GetCalculatePosition(planePos, CurrentCamera.CFrame.LookVector, ray.Origin, ray.Direction)
        blurPart.CFrame = CFrame.new(hit)
        blurPart.Size = Vector3.new(size.X/50, size.Y/50, 0.5)
    end

    -- Atualiza a cada frame (ou quando mover)
    local conectar = game:GetService("RunService").RenderStepped:Connect(updateBlur)
    NeverLose:AddSignal(conectar)

    -- ======== DRAG ========
    NeverLose.Drag(icone, icone, 0.15)  -- arrasta o próprio frame

    -- ======== CLICK ========
    local btn = Instance.new("ImageButton", icone)
    btn.Size = UDim2.fromScale(1,1)
    btn.BackgroundTransparency = 1
    btn.ImageTransparency = 1
    btn.MouseButton1Click:Connect(function()
        window:ToggleInterface()  -- abre/fecha a UI
    end)

    -- ======== LIMPEZA ========
    icone.Destroying:Connect(function()
        blurPart:Destroy()
        blurEffect:Destroy()
        conectar:Disconnect()
    end)
...
-- ====================================================
-- USO: depois de criar a window, chame:
-- ====================================================
local window = NeverLose:CreateWindow({ ... })  -- tua window
CriarIconeFlutuante(window)
