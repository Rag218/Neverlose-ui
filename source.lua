...
--[[
    Draggable Toggle Icon – always on top, square with borders, blur glow.
    Config:
        Icon   : string (image asset ID or custom asset path) – default GlobalLogo
        Size   : number (pixels) – default 50
        Border : number (pixels) – default 2
        Color  : Color3 – default AccentColor
        Blur   : boolean – enable background blur behind icon (default true)
]]
function NeverLose:CreateToggleIcon(Window, Config)
    ...
    local UIStroke = Instance.new("UIStroke")
    local ImageLabel = Instance.new("ImageLabel")
    local Shadow = NeverLose:CreateShadow(IconFrame, true)  -- rolling glow effect
    ...
    IconFrame.Position = UDim2.fromOffset(20, 20)   -- top‑left corner
    IconFrame.ZIndex = 999   -- always on top

    -- Rounded corners
    UICorner.CornerRadius = UDim.new(0, 8)
    UICorner.Parent = IconFrame

    -- Border with your chosen color
    ...
    -- Dragging logic (reuses your existing Drag function)
    ...
    ClickBtn.MouseEnter:Connect(hoverIn)
    ClickBtn.MouseLeave:Connect(hoverOut)

    -- Optional Blur behind the icon (always on)
    if Config.Blur then
        ...
        depth.NearIntensity = 0.85   -- subtle blur

        -- Update blur position to follow icon
        local function updateBlur()
            ...
            local normX = center.X / screenSize.X
            local normY = center.Y / screenSize.Y
            -- Map to 3D world (approximate)
            local ray = workspace.CurrentCamera:ViewportPointToRay(center.X, center.Y, 1)
            local planeDist = 5
            local worldPos = ray.Origin + ray.Direction * planeDist
        ...
        -- Cleanup when icon is destroyed
    ...
    -- Return the icon object so you can later change its properties
    return {
        Frame = IconFrame,
        Image = ImageLabel,
